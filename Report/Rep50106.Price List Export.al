report 50106 "Price List Export"
{
    ...
    dataset
    {
        ...
        {
            RequestFilterFields = "Product No.", "Price Type", "Assign-to No.", "Starting Date", "Ending Date";
            DataItemTableView = where("Asset Type" = const(Item));

            trigger OnPreDataItem()
            begin

                if PriceTypeFilter = G_PriceType_Sales then begin
            ...
            begin
                if not FormatOnly then begin
                    ...
                    if PriceTypeFilter = G_PriceType_Sales then begin
                        ...
                        if PriceLineCombi.FindFirst() then begin
                            CombiFound := true;
                        end;
    ...
    begin
        ...
        G_PriceType_Sales := Format(Enum::"Price Type"::Sale);
        G_PriceType_Purch := Format(Enum::"Price Type"::Purchase);
    end;

    trigger OnPreReport()
    begin
        if not FormatOnly then begin
            PriceTypeFilter := "PriceList".GetFilter("Price Type");
            if (PriceTypeFilter <> G_PriceType_Sales)
                and (PriceTypeFilter <> G_PriceType_Purch) then begin
                Error('Please select either Sale or Purchase as the Price Type.');
            end;
        end;
    ...
    var
        ...
        G_PriceType_Sales: Text[20];
        G_PriceType_Purch: Text[20];
        PriceTypeFilter: Text[20];
