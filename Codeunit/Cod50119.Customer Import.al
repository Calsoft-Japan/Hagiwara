codeunit 50119 "Customer Import"
{
    ...
    var
        ...
        Prepayment: Decimal;
        PartnerTypeStr: Text;
        //PreferredBankAccountCodeStr: Text;//Comment the code as required on 02/12/2026
    ...
    begin
        ...
        for RowNo := 2 to MaxRowNo do begin
            ...
            */
            CashFlowPaymentTermsCodeStr := GetValueAtCell(RowNo, 50).Trim();
            PrimaryContactNoStr := GetValueAtCell(RowNo, 51).Trim();
            ResponsibilityCenterStr := GetValueAtCell(RowNo, 52).Trim();
            ShippingAdviceStr := GetValueAtCell(RowNo, 53).Trim().ToUpper();
            ShippingTimeStr := GetValueAtCell(RowNo, 54).Trim();
            ShippingAgentServiceCodeStr := GetValueAtCell(RowNo, 55).Trim();
            ServiceZoneCodeStr := GetValueAtCell(RowNo, 56).Trim();
            ContractGainLossAmountStr := GetValueAtCell(RowNo, 57).Trim();
            AllowLineDiscStr := GetValueAtCell(RowNo, 58).Trim();
            CopySelltoAddrtoQteFromStr := GetValueAtCell(RowNo, 59).Trim().ToUpper();
            CustomerTypeStr := GetValueAtCell(RowNo, 60).Trim().ToUpper();
            NECOEMCodeStr := GetValueAtCell(RowNo, 61).Trim();
            NECOEMNameStr := GetValueAtCell(RowNo, 62).Trim();
            ShippingMark1Str := GetValueAtCell(RowNo, 63).Trim();
            ShippingMark2Str := GetValueAtCell(RowNo, 64).Trim();
            ShippingMark3Str := GetValueAtCell(RowNo, 65).Trim();
            ShippingMark4Str := GetValueAtCell(RowNo, 66).Trim();
            ShippingMark5Str := GetValueAtCell(RowNo, 67).Trim();
            Remarks1Str := GetValueAtCell(RowNo, 68).Trim();
            Remarks2Str := GetValueAtCell(RowNo, 69).Trim();
            Remarks3Str := GetValueAtCell(RowNo, 70).Trim();
            Remarks4Str := GetValueAtCell(RowNo, 71).Trim();
            Remarks5Str := GetValueAtCell(RowNo, 72).Trim();
            ItemSupplierSourceStr := GetValueAtCell(RowNo, 73).Trim().ToUpper();
            VendorCustCodeStr := GetValueAtCell(RowNo, 74).Trim();
            ShipFromNameStr := GetValueAtCell(RowNo, 75).Trim();
            ShipFromAddressStr := GetValueAtCell(RowNo, 76).Trim();
            HQTypeStr := GetValueAtCell(RowNo, 77).Trim();
            DefaultCountryRegionofOrgStr := GetValueAtCell(RowNo, 78).Trim().ToUpper();
            PriceUpdateTargetDateStr := GetValueAtCell(RowNo, 79).Trim().ToUpper();
            ORECustomerNameStr := GetValueAtCell(RowNo, 80).Trim();
            OREAddressStr := GetValueAtCell(RowNo, 81).Trim();
            OREAddress2Str := GetValueAtCell(RowNo, 82).Trim();
            ORECityStr := GetValueAtCell(RowNo, 83).Trim();
            OREStateProvinceStr := GetValueAtCell(RowNo, 84).Trim();
            ExcludedinORECollectionStr := GetValueAtCell(RowNo, 85).Trim();
            ORECountryStr := GetValueAtCell(RowNo, 86).Trim();
            OREPostCodeStr := GetValueAtCell(RowNo, 87).Trim();
            CustomerGroupStr := GetValueAtCell(RowNo, 88).Trim();
            FamiliarNameStr := GetValueAtCell(RowNo, 89).Trim();
            ImportFileShipToStr := GetValueAtCell(RowNo, 90).Trim();
            ...
            //rec_POInt."Preferred Bank Account Code" := PreferredBankAccountCodeStr;  //Comment the code as required on 02/12/2026
