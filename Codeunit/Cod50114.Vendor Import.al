codeunit 50114 "Vendor Import"
{
    ...
    begin
        ...
        for RowNo := 2 to MaxRowNo do begin
            ...
            //Partner Type
            if ((PartnerTypeStr = '') or (not (PartnerTypeStr in ['Company', 'Person', 'Government']))) then begin
                Error(PartnerTypeMsg);
            end;
            //Update PO Price Target Date
            if ((UpdatePOriceTargetDateStr = '') or (not (UpdatePOriceTargetDateStr in ['Order Date', 'Expected Receipt Date']))) then begin
                Error(UpdatePOPriceTargetDateMsg);
            end;
            //Blocked
            if ((BlockedStr = '') or (not (BlockedStr in ['Ship', 'Invoice', 'All']))) then begin
                Error(BlockedMsg);
            end;
            ...
            Evaluate(rec_POInt."Preferred Bank Account Code", GetValueAtCell(RowNo, 46));
            Evaluate(rec_POInt."Cash Flow Payment Terms Code", GetValueAtCell(RowNo, 47));
            Evaluate(rec_POInt."Primary Contact No.", GetValueAtCell(RowNo, 48));
            Evaluate(rec_POInt."Responsibility Center", GetValueAtCell(RowNo, 49));
            Evaluate(rec_POInt."Location Code", GetValueAtCell(RowNo, 50));
            Evaluate(rec_POInt."Lead Time Calculation", GetValueAtCell(RowNo, 51));
            Evaluate(rec_POInt."ID No.", GetValueAtCell(RowNo, 52));
            Evaluate(rec_POInt."Shipping Terms", GetValueAtCell(RowNo, 53));
            Evaluate(rec_POInt."Incoterm Code", GetValueAtCell(RowNo, 54));
            Evaluate(rec_POInt."Incoterm Location", GetValueAtCell(RowNo, 55));
            Evaluate(rec_POInt."Manufacturer Code", GetValueAtCell(RowNo, 56));
            Evaluate(rec_POInt."ORE Reverse Routing Address", GetValueAtCell(RowNo, 57));
            Evaluate(rec_POInt."Excluded in ORE Collection", GetValueAtCell(RowNo, 58));
            Evaluate(rec_POInt."ORE Reverse Routing Address SD", GetValueAtCell(RowNo, 59));
            Evaluate(rec_POInt."Hagiwara Group", GetValueAtCell(RowNo, 60));
            Evaluate(rec_POInt."Familiar Name", GetValueAtCell(RowNo, 61));
            Evaluate(rec_POInt."Pay-to Address", GetValueAtCell(RowNo, 62));
            Evaluate(rec_POInt."Pay-to Address 2", GetValueAtCell(RowNo, 63));
            Evaluate(rec_POInt."Pay-to City", GetValueAtCell(RowNo, 64));
            Evaluate(rec_POInt."Pay-to Post Code", GetValueAtCell(RowNo, 65));
            Evaluate(rec_POInt."Pay-to County", GetValueAtCell(RowNo, 66));
            Evaluate(rec_POInt."Pay-to Country/Region Code", GetValueAtCell(RowNo, 67));
            Evaluate(rec_POInt."Exclude Check", GetValueAtCell(RowNo, 68));
